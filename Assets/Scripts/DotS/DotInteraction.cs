...
public class DotInteraction : MonoBehaviour
{
    ...
    public float swipeLengthCanMove;
    ...
    private void FindMatches()
    {
        if (this.column > 0 && this.column < this.allDot.Width - 1)
        {
            ...
            if (leftDot && rightDot && leftDot != this.gameObject && rightDot != gameObject)
            {
    ...
    private string SwipeDirection()
    {
        ...
        if (Vector2.Distance(this.mouseUpPos, this.mouseDownPos) < 80)
        {
            GameStateController.Instance.CurrentState = GameStates.Swipe;
            Debug.Log("false");
        ...
        Debug.Log("0");
        Debug.Log("Góc giữa AB và trục x dương: " + angleMouseVector + " độ.");
        return swipeDirection;
